...
for cent in range(Total_centers):
    ...
    for _sess in range(len(this_session)):
        print("{0:^12} {1:^12} {2:^14} {3:^16} ".format(
            this_session[_sess]['date'], this_session[_sess]['vaccine'], this_session[_sess]['min_age_limit'], this_session[_sess]['available_capacity']))
